...
class _DevisPageState extends ConsumerState<DevisPage> {
  ...
  Future<void> _saveSignature({
    ...
    if (!_signatureController.hasSignature) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Aucune signature Ã  enregistrer.')),
      );
      return;
    }
  ...
  Future<void> _sendToClient({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Devis prÃªt Ã  Ãªtre envoyÃ©.')),
      );
    } catch (error) {
      if (!mounted) {
        return;
  ...
  Future<void> _generateQuoteReport({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Rapport devis généré : ${report.title}'),
        ),
      );
    } catch (error) {
      if (!mounted) {
          ...
          child: selectedProjectAsync.when(
            data: (project) {
              ...
              return quoteAsync.when(
                          ...
                          if (isCompact)
                            Column(
                              children: [
                                TextFormField(
                                  initialValue:
                                      unitPriceWalls.toStringAsFixed(0),
                                  decoration: const InputDecoration(
                                    labelText: 'PU peinture murs',
                                  ),
                                  onChanged: (value) {
                                    unitPriceWalls = double.tryParse(value) ??
                                        unitPriceWalls;
                                  },
                                ),
                                const SizedBox(height: 12),
                                TextFormField(
                                  initialValue:
                                      unitPriceCeiling.toStringAsFixed(0),
                                  decoration: const InputDecoration(
                                    labelText: 'PU peinture plafond',
                                  ),
                                  onChanged: (value) {
                                    unitPriceCeiling = double.tryParse(value) ??
                                        unitPriceCeiling;
                                  },
                                ),
                              ],
                                  ...
                                  Wrap(
                                    ...
                                    children: [
                                      FilledButton.icon(
                                        ...
                                        label:
                                            const Text('Enregistrer signature'),
                                      ),
                                      OutlinedButton.icon(
                                ...
                                children: [
                                  Wrap(
                                    ...
                                    children: [
                                      ...
                                      OutlinedButton.icon(
                                        onPressed: () => _generateQuoteReport(
                                          context: context,
                                          projectId: project.id,
                                          projectName: project.name,
                                        ),
                                  ...
                                  for (final item in items)
                                    ListTile(
                                      ...
                                      subtitle: Text(
                                        '${item.quantity.toStringAsFixed(2)} ${item.unit} × ${item.unitPrice.toStringAsFixed(2)}',
                                      ),
                                      trailing: Text(
                ...
                error: (error, _) =>
                    Center(child: Text('Erreur devis: $error')),
              );
            },
            loading: () => const Center(child: CircularProgressIndicator()),
