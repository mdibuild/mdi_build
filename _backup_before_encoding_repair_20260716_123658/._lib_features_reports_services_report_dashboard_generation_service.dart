...
class ReportDashboardGenerationService {
  ...
  Future<_ReportPayload> _buildGlobalPayload({
    ...
    return _ReportPayload(
      progressSummary:
          'Rapport global gÃƒÂ©nÃƒÂ©rÃƒÂ© pour ${_date(periodStart)} au ${_date(periodEnd)}.',
      sections: sections,
      sources: sources,
    );
  }
  ...
  Future<_ReportPayload> _buildProjectPayload({
    ...
    final section = _textSection(
      ...
      sectionTitle: 'SynthÃƒÂ¨se projet',
      sortOrder: 10,
      contentText: [
        'Projet : $projectName',
        'PÃƒÂ©riode : ${_date(periodStart)} au ${_date(periodEnd)}',
        'Statut : ${project['status'] ?? '-'}',
        'Client : ${project['client_name'] ?? project['client'] ?? '-'}',
        'DerniÃƒÂ¨re mise ÃƒÂ  jour : ${_safeDateTime(project['updated_at'])}',
      ].join('\n'),
    ...
    return _ReportPayload(
      progressSummary: 'SynthÃƒÂ¨se projet disponible.',
      sections: [section],
      sources: [source],
    );
  }
  ...
  Future<_ReportPayload> _buildPlanningPayload({
    ...
    final section = _textSection(
      ...
      sectionTitle: 'SynthÃƒÂ¨se planning',
      sortOrder: 10,
      contentText: [
        'Projet : $projectName',
        'PÃƒÂ©riode : ${_date(periodStart)} au ${_date(periodEnd)}',
        'Total tÃƒÂ¢ches : ${tasks.length}',
        'ÃƒDA faire : $todo',
        'En cours : $inProgress',
        'TerminÃƒÂ©es : $done',
        'En retard : $late',
    ...
    return _ReportPayload(
      progressSummary:
          'SynthÃƒÂ¨se planning gÃƒÂ©nÃƒÂ©rÃƒÂ©e sur ${tasks.length} tÃƒÂ¢che(s).',
      sections: [section],
      sources: [source],
    );
  }
  ...
  Future<_ReportPayload> _buildQuotesPayload({
    ...
    return _ReportPayload(
      progressSummary:
          'SynthÃƒÂ¨se devis gÃƒÂ©nÃƒÂ©rÃƒÂ©e sur ${quotes.length} devis.',
      sections: [section],
      sources: [source],
    );
  }
  ...
  Future<_ReportPayload> _buildPurchasesPayload({
    ...
    final section = _textSection(
      ...
      sectionTitle: 'SynthÃƒÂ¨se achats',
      sortOrder: 10,
      contentText: [
        'Projet : $projectName',
        'PÃƒÂ©riode : ${_date(periodStart)} au ${_date(periodEnd)}',
        ...
        'Brouillons : $draft',
        'CommandÃƒÂ©s : $ordered',
        'LivrÃƒÂ©s : $delivered',
        'Montant total estimÃƒÂ© : ${totalAmount.toStringAsFixed(2)}',
      ].join('\n'),
      contentJson: <String, dynamic>{
    ...
    return _ReportPayload(
      progressSummary:
          'SynthÃƒÂ¨se achats gÃƒÂ©nÃƒÂ©rÃƒÂ©e sur ${purchases.length} achat(s).',
      sections: [section],
      sources: [source],
    );
  }
  ...
  Future<_ReportPayload> _buildMetragePayload({
    ...
    final section = _textSection(
      ...
      sectionTitle: 'SynthÃƒÂ¨se mÃƒÂ©trÃƒÂ©',
      sortOrder: 10,
      contentText: [
        'Projet : $projectName',
        'PÃƒÂ©riode : ${_date(periodStart)} au ${_date(periodEnd)}',
    ...
    return _ReportPayload(
      progressSummary:
          'SynthÃƒÂ¨se mÃƒÂ©trÃƒÂ© gÃƒÂ©nÃƒÂ©rÃƒÂ©e sur ${spaces.length} espace(s).',
      sections: [section],
      sources: [source],
    );
  }
