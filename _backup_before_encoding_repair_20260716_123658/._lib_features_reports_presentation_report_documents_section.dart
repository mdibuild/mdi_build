...
class _ReportDocumentsSectionState
  ...
  Future<void> _pickAndUploadDocument() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Document ajoutÃƒÆ’Ã‚Â© au rapport.')),
      );
    } catch (error) {
      if (!mounted) {
        return;
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text(
                        _uploading
                            ? 'Ajout...'
                            : 'Ajouter une piÃƒÆ’Ã‚Â¨ce jointe',
                      ),
                    ),
                  ),
                ],
              ...
              builder: (context, snapshot) {
                ...
                if (documents.isEmpty) {
                  return const Padding(
                    padding: EdgeInsets.all(12),
                    child:
                        Text('Aucun document liÃƒÆ’Ã‚Â© ÃƒÆ’Ã‚Â  ce rapport.'),
                  );
                }
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        'CatÃƒÆ’Ã‚Â©gorie : ${_categoryLabel(document.category)} ÃƒâDAšÃ‚Â· AjoutÃƒÆ’Ã‚Â© le $createdAt',
...
class _DocumentMetadataDialogState extends State<_DocumentMetadataDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('Nouvelle piÃƒÆ’Ã‚Â¨ce jointe'),
      content: SizedBox(
        width: 520,
        child: Form(
          key: _formKey,
