...
class _DevisPageState extends ConsumerState<DevisPage> {
  ...
  Future<void> _openGeneratePurchaseDialog(
    ...
    final purchaseItems = items
        ...
        .map(
          (entry) => PurchaseItem(
            ...
            description: 'Ligne gÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©rÃƒÆ’Ã‚Â©e depuis le devis',
    ...
    final draftPurchase = Purchase(
      ...
      companyId: profile.companyId,
      projectId: projectId,
      title: 'Achat gÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©rÃƒÆ’Ã‚Â© depuis devis',
      lot: mode == 'corps_etat'
          ? 'Corps dÃƒÂ¢DAÃ¢âDAžÂ¢ÃƒÆ’Ã‚Â©tat'
          : 'PiÃƒÆ’Ã‚Â¨ce',
      notes: 'GÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©rÃƒÆ’Ã‚Â© depuis le devis de $projectName',
      status: 'brouillon',
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          title: const Text('GÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©rer achat depuis le devis'),
          content: SizedBox(
            width: 760,
            child: PurchaseForm(
              projectOptions: [
              ...
              onSubmit: (value) async {
                ...
                if (dialogContext.mounted) {
                  ScaffoldMessenger.of(dialogContext).showSnackBar(
                    const SnackBar(
                      content: Text(
                          'Achat gÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©rÃƒÆ’Ã‚Â© depuis le devis.'),
                    ),
                  );
                }
              },
  ...
  Future<void> _saveSignature({
    ...
    if (!_signatureController.hasSignature) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Aucune signature ÃƒÆ’Ã‚Â  enregistrer.')),
      );
      return;
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Signature enregistrÃƒÆ’Ã‚Â©e.')),
    );
  }
  ...
  Future<void> _clearSignature({
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Signature effacÃƒÆ’Ã‚Â©e.')),
    );
  }
  ...
  Future<void> _saveDraft({
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Devis enregistrÃƒÆ’Ã‚Â©.')),
    );
  }
  ...
  Future<void> _sendToClient({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content:
                Text('Devis prÃƒÆ’Ã‚Âªt ÃƒÆ’Ã‚Â  ÃƒÆ’Ã‚Âªtre envoyÃƒÆ’Ã‚Â©.')),
      );
    } catch (error) {
      if (!mounted) {
        return;
                        ...
                        children: [
                          DropdownButtonFormField<String>(
                            ...
                            items: const [
                              DropdownMenuItem(
                                value: 'piece',
                                child: Text('Par piÃƒÆ’Ã‚Â¨ce'),
                              ),
                              DropdownMenuItem(
                                value: 'corps_etat',
                                child: Text(
                                    'Par corps dÃƒÂ¢DAÃ¢âDAžÂ¢ÃƒÆ’Ã‚Â©tat'),
                              ),
                              DropdownMenuItem(
                                value: 'mixte',
                                child: Text('Mixte'),
                          ...
                          DropdownButtonFormField<String>(
                            ...
                            items: const [
                              ...
                              DropdownMenuItem(
                                value: 'envoye',
                                child: Text('EnvoyÃƒÆ’Ã‚Â©'),
                              ),
                              DropdownMenuItem(
                                value: 'signe',
                                child: Text('SignÃƒÆ’Ã‚Â©'),
                              ),
                            ],
                            onChanged: (value) {
                              if (value != null) {
                            ...
                            Row(
                              children: [
                                ...
                                if (savedSignatureBytes != null)
                                  const Chip(label: Text('EnregistrÃƒÆ’Ã‚Â©e')),
                              ],
                            ),
                            const SizedBox(height: 12),
                            RepaintBoundary(
                              ...
                              children: [
                                ...
                                for (final item in items)
                                  ListTile(
                                    ...
                                    subtitle: Text(
                                      '${item.quantity.toStringAsFixed(2)} ${item.unit} ÃƒÆ’Ã¢DAâDA ${item.unitPrice.toStringAsFixed(2)}',
                                    ),
                                    trailing:
  ...
  String _modeLabel(String value) {
    switch (value) {
      case 'piece':
        return 'Par piÃƒÆ’Ã‚Â¨ce';
      case 'corps_etat':
        return 'Par corps dÃƒÂ¢DAÃ¢âDAžÂ¢ÃƒÆ’Ã‚Â©tat';
      case 'mixte':
        return 'Mixte';
      default:
        return value;
